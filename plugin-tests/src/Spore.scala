...
abstract class ActorRef[T,-W[_]] {
  /*
   * Argument cont is in a contravariant position.
   * This means that implementations of ActorRef must not
   * have a more specific refinement of the Spore type.
   */
  def send(msg: Box[T])(cont: Spore[Unit,Unit] { type Excluded = msg.C; type CapturingWitness[A] = W[A] })
          (implicit acc: CanAccess { type C = msg.C }): Nothing
}

class NoReturnControl extends ControlThrowable {
...
object Box {
  ...
  private def packedNull[S] = new Packed[S] {
...
object Safe {
  ...
  implicit def actorRefIsSafe[T,W]: Safe[ActorRef[T,W]] = new Safe[ActorRef[T,W]] {}
...
sealed class Box[+T] private (private val instance: T) {
  ...
  def swap[S](select: T => Box[S])(assign: (T, Box[S]) => Unit, newBox: Box[S])(
    ...
    if (prev == null) {
      ...
      implicit def fakeSafe[T]: Safe[T] = null
