...
type Lifter[F <: Witness] = [T] =>> Exorcist[T,F[T]]
...
object FreshOnce {
  ...
  implicit def shallowCloner[A: Lifter[Immutable],CC[A] <: Cloneable[CC[A]]]: Exorcist[CC[A],FreshOnce[CC[A]]] =
    new Exorcist {
  ...
  implicit def deepCloner[A,CC[A] <: IterableOnceOps[A,CC,CC[A]]](implicit ev: Lifter[FreshOnce][A]): Lifter[FreshOnce][CC[A]] =
    new Exorcist {
      override def evictDemons(x: CC[A]): FreshOnce[CC[A]] =
        new FreshOnce[CC[A]] {
...
object Fresh {
  ...
  import scala.collection.mutable._
  implicit def shallowCloner[A: Lifter[Immutable],CC[A] <: Cloneable[CC[A]]]: Lifter[Fresh][CC[A]] =
    new Exorcist {
  ...
  implicit def deepCloner[A,CC[A] <: IterableOnceOps[A,CC,CC[A]]](implicit ev: Lifter[Fresh][A]): Lifter[Fresh][CC[A]] =
    new Exorcist {
      override def evictDemons(x: CC[A]): Fresh[CC[A]] =
        new Fresh[CC[A]] {
...
object Serialized {
  ...
  implicit def serializer[A <: Serializable]: Lifter[Serialized][A] =
    new Exorcist {
...
object Immutable {
  ...
  def apply[T](x: T)(implicit exorcist: Lifter[Immutable][T]): Immutable[T] = {
    exorcist.evictDemons(x)
  }
  def unapply[T](pure: Immutable[T]): Some[T] = Some(pure.extract)
  def unapply[T](x: T)(implicit ev: Lifter[Immutable][T]): Some[T] = Some(x)
...
object Frozen {
  ...
  implicit def BlessedFreezer[T](implicit blesser: Lifter[Immutable][T]): Lifter[Frozen][T] = blesser
