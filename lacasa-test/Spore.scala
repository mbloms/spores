...
object Spore {
  def spore[A,B,E,W[_]](fun: A => B): Spore[A, B] {
    type Excluded = E
    type CapturingWitness = W
  } = new Spore[A,B](fun) {
    type Excluded = E
    type CapturingWitness = W}
}
...
abstract class ActorRef[T] {
   ...
   */
  def send(msg: Box[T])(cont: Spore[Unit,Unit] { type Excluded = msg.C; type CapturingWitness[T] = Immutable[T]})
